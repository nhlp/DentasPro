namespace Dentas_Pro.UI.User
{
    ...
    public partial class History_Of_Annual_Premit_Form : Base_Form
    {
        public History_Of_Annual_Premit_Form()
        {
            InitializeComponent();
            ...
            Show_All_Annual_Permit_On_UI();
        }

        /*#########Bussiness Process#######
         * Yıllık izin bakiyelerinin hareketleri. 
         *  İzin harektleri nelerdir.
         *      Devir bakiyesi
         *      Yeni izin hakediş    
         *      IK izin ekleme çıkarma
         *      Yıllık izin formlarından doğan harektler.
         #########Bussiness Process#######*/    
        ...
        public void Getting_For_Open_Up_User_Name_Surname_Instead_Obj_Id(int Obj_Id)
        {
            try
            {
                ...
                This_Year_Permit_Count_TextEdit.Text = user.This_Year_Permit_Count.ToString();
                ...
                if (Current_Year > Job_Enterence_Year && Current_Month > Job_Enterence_Month || Current_Month == Job_Enterence_Month && Current_Day > Job_Enterence_Day && Date_6 <= (TimeSpan.Parse("1825")))
                {
                    ...
                    {
                        This_Year_Permit_Count_TextEdit.Text = "14";
                        user.Accumulated_Permit_count = user.Accumulated_Permit_count + Int16.Parse(This_Year_Permit_Count_TextEdit.Text);
                        Accumulated_Permit_Count_TextEdit.Text = user.Accumulated_Permit_count.ToString();
                        This_Year_Permit_Count_TextEdit.Text = "0";
                ...
                if (Current_Year > Job_Enterence_Year && Current_Month > Job_Enterence_Month || Current_Month == Job_Enterence_Month && Current_Day > Job_Enterence_Day &&  Date_6 >= (TimeSpan.Parse("1825")) || Date_6 >= (TimeSpan.Parse("5475")))
                {
                    ...
                    if (Assigned_Year <= Permit_Right_Year && user.Assigned_Permit != -1)
                    {
                        This_Year_Permit_Count_TextEdit.Text = "20";
                        user.Accumulated_Permit_count = user.Accumulated_Permit_count + Int16.Parse(This_Year_Permit_Count_TextEdit.Text);
                        Accumulated_Permit_Count_TextEdit.Text = user.Accumulated_Permit_count.ToString();
                        This_Year_Permit_Count_TextEdit.Text = "0";
                        user.Assigned_Permit = -1;
                ...
                if (Current_Year > Job_Enterence_Year && Current_Month > Job_Enterence_Month || Current_Month == Job_Enterence_Month && Current_Day > Job_Enterence_Day && Date_6 >= (TimeSpan.Parse("5475")))
                {
                    ...
                    if (Assigned_Year <= Permit_Right_Year && user.Assigned_Permit != -1)
                    {
                        This_Year_Permit_Count_TextEdit.Text = "26";
                        user.Accumulated_Permit_count = user.Accumulated_Permit_count + Int16.Parse(This_Year_Permit_Count_TextEdit.Text);
                        Accumulated_Permit_Count_TextEdit.Text = user.Accumulated_Permit_count.ToString();
                        This_Year_Permit_Count_TextEdit.Text = "0";
                        user.Assigned_Permit = -1;
        ...
        public string Annual_Permit_Calculating(int Obj_Id)
        {
            ...
            return This_Year_Permit_Count_TextEdit.Text;
        ...
        /// <summary>
        /// Yıllık hak edilecek izin hesaplaması yapıyoruz.
        /// </summary>
        //public void Calculate_The_Annual_Permit()
        //{
        //}
        ...
        public void Show_All_Annual_Permit_On_UI() 
        {
            var list = from hap in sql_Server_Db_From_Follow_Up_Data_context.History_Of_Annual_Permits

                       join u in sql_Server_Db_From_Follow_Up_Data_context.Users on hap.User_Obj_Id equals u.Obj_Id
                       //join d in sql_Server_Db_From_Follow_Up_Data_context.Departments on hap.Department_Obj_Id equals d.Obj_Id
                       ///*join pr in sql_Server_Db_From_Follow_Up_Data_Context.Permit_Reasons on ap.Permit_Reason_Obj_Id equals pr.Obj_Id*/
                       orderby hap.Obj_Id descending
                       /*Where clause'unda bilgi işlem'i filtreliyoruz*/
                       //where u.Department_Obj_Id == annual_Permit_Form.Department_Obj_Id && hap.Approval_Department_Manager_Situation == false || hap.Approval_Department_Manager_Situation == null
                       select new
                       {
                           Sıra = hap.Obj_Id,
                           Ad_Soyad = u.Full_Name,
                           Sicil_Numarası = u.Registration_Number,
                           Açıklama = hap.Description,
                           Bakiye  = hap.Permit_Rest
                           /*u.Department_Obj_Id.Value,*/
                           //Departman_Adı = d.Name,
                           //İzin_Başlangıç_Tarihi = hap.Permit_Start_Time,
                           //İzin_Bitiş_Tarihi = hap.Permit_End_Time,
                           /*ap.Permit_Type,*/
                           /*İzin_Sebebi = pr.Name,*/
                           //Bu_yıl_Hak_Edilen_İzin = u.This_Year_Permit_Count,
                           //Kullanılmış_İzin_Gün_Adedi = hap.Permitted_Use_Knowledge_Data,
                           //Bakiye = hap.Accumulated_Permit_count_Knowledge_Data
                       };
